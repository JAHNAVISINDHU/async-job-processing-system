...
- **API health**
  ...
  - `GET http://localhost:3000/health` – readiness check used by health checks and integration tests.

- **API base**

  - Base URL: `http://localhost:3000`

  - `POST /jobs` – create a job (CSV or email):
  ...
  - `GET /jobs/:id` – fetch job details and status for a given `jobId`.

  - Admin endpoints:

    - `GET /admin/jobs` – list jobs
    - `GET /admin/failed` – list failed jobs
    - `POST /admin/jobs/:id/retry` – retry a specific job
    - `POST /admin/jobs/:id/dlq` – move a job to the dead-letter queue

- **Admin UI (web)**

  - `http://localhost:3000/admin/ui` – minimal management UI for failed jobs.
...
- **Output directory (host)**
  ...
  - `./output` – CSV files generated by `CSV_EXPORT` jobs, named `{jobId}.csv`.
...
- `docker-compose.yml` – orchestrates API, worker, PostgreSQL, Redis, and MailHog containers.
- `seeds/01_init.sql` – DB init script to create the `jobs` table and related schema.
- `src/server.js` – Express API server (job and admin endpoints).
- `src/worker.js` – background worker (pulls jobs from Redis and processes them).
- `output/` – shared volume for generated CSV artifacts.

---

## Integration Tests ✅

Integration tests exercise the full stack (API → worker → DB/Redis → output/MailHog).
